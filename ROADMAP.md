...
- [ ] Move the 12 models from `contracts/models/*.py` into a single `app/models.py`. They are
      already written and tested — this is a merge, not a rewrite. Verify: `python -c "import app.models"` exits 0.
- [ ] Delete `frontend/`, `backend/`, `contracts/`, `services/`, `datagen/`, `render.yaml`,
      `.python-version`. One app now. ⛔ blocks: everything
- [ ] `app/db.py`: `sqlite3` schema for studies, sites, subjects, visits, deviations, queries,
      adverse_events, milestones, alerts, audit_events. Verify: `python -m app.db --init` creates
      `data/ctms.db` and `.tables` lists 10 tables.
- [ ] `app/db.py` seeds from `app/datagen.py` on startup if the DB is empty. Verify: delete
      `data/ctms.db`, start the app, the portfolio page has data.
- [ ] `app/main.py`: FastAPI with Jinja2 templates mounted, `/health` returns 200, `/` renders
      `base.html` with the app title. Verify: `uvicorn app.main:app` then open localhost:8000.
- [ ] `app/templates/base.html`: Tailwind CDN, nav to the four screens, "synthetic data" footer.
...
- [ ] `/portfolio` — six KPI tiles (active studies, enrolled vs target, sites activated, open
...
- [ ] `/study/{id}` — enrolment vs target, site list, milestone timeline, deviation and query
      counts. Verify: clicking a study row from the portfolio lands here with the right data.
- [ ] `/ae` — AE list plus an intake form that POSTs and redirects back with the new row visible.
- [ ] `/audit` — chronological table: actor, action, resource, before/after, UTC timestamp.
- [ ] Every mutating route writes an audit row. Verify: file an AE, see that exact event on
      `/audit` with `after` populated. ⛔ blocks: Phase 2 audit work
- [ ] Nav links between all four screens work. No dead ends, no 404s.
...
- [ ] Hash chain: each row stores `prev_hash` and `sha256(canonical_json(payload) + prev_hash)`.
- [ ] `python -m app.audit --verify` walks the chain and prints OK, or the sequence number of the
...
- [ ] SQLite trigger raising on UPDATE or DELETE of `audit_events`, so the guarantee does not
      depend on application code. Verify: the UPDATE above fails at the DB level.
- [ ] `/audit` gets a **Verify chain** button calling the verifier and showing the result.

### Pharmacovigilance
- [ ] `app/pv.py` codes a free-text narrative against `terms.csv` (normalise, exact, then
      `difflib` fuzzy). Verify: `python -m app.pv "patient had a bad headache"` returns Headache.
- [ ] Coding results carry a `source` field reading `"curated"` — never implies MedDRA.
- [ ] NDCT Rules 2019 clocks: SAE → EC/licensing authority within 24h, narrative within 14d,
      computed from the server clock on intake. Verify: an SAE with a past onset shows BREACHED
      with negative hours remaining.
- [ ] AE intake form shows coding suggestions and the countdown after submit.

### KPIs and alerts
- [ ] KPI computation reads SQLite: enrolment %, screen-failure rate, visit compliance,
      open-query ageing, deviation rate, days to next milestone.
- [ ] Three alert rules from `config.py` thresholds: enrolment lag, EC renewal due, monitoring
      visit overdue. Verify: change `ALERT_ENROLMENT_LAG_PCT`, restart, the alert count changes.
- [ ] Alerts render on `/portfolio` severity-ranked, each linking to its study.

### Data
- [ ] `app/datagen.py` produces a portfolio that tells a story: 8 studies, 12 sites, ~400
...
- [ ] Fixed seed. Verify: regenerating twice produces identical numbers. The demo, the deck and
...
- [ ] `docs/demo-script.md` — the exact click path, in order, with what you say on each screen.
- [ ] `docs/compliance-matrix.md` — every framework as a row: GCP-ASU, ICMR guidelines, NDCT 2019,
...
- [ ] `docs/qa-prep.md` — 12 questions with answers. Lead with "is that real MedDRA?" and "is this
      real patient data?"
- [ ] Deck locked: problem, architecture, live demo, audit trail, pharmacovigilance, compliance
      matrix, deferred scope.
- [ ] Sleep 90 minutes. Q&A performance is a function of sleep and you have no teammate to cover.
...
- [ ] **"View as" role selector** — a session value switching the portfolio between Principal
      Investigator, coordinator, pharmacovigilance and regulator views. Presentation only, no
      enforcement. The problem statement names seven role-based views, so this is the answer to a
      question you will otherwise be asked with nothing to show.
- [ ] DSMB signal view: AE counts by coded term × study × severity, ranked.
- [ ] SDTM-shaped DM export as CSV. Columns: STUDYID DOMAIN USUBJID SUBJID SITEID AGE SEX ARM RFSTDTC.
- [ ] Audit log filters by actor and date range.
...
- [ ] Visual pass on the three screens that appear in the demo. Nothing else gets touched.
- [ ] FHIR R4 `ResearchStudy` JSON at `/api/fhir/ResearchStudy/{id}`. Structure only.

---

# Phase 5 — Rehearse · Hours 21–24

- [ ] Three clean end-to-end runs. Not two.
- [ ] Time the demo. Four minutes means four minutes.
- [ ] Answer all 12 Q&A questions out loud, standing up.
